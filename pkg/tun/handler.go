...
func (h *handler) injectAndForward(appConn, serverConn net.Conn, dst string) {
	...
	seq, ack, hopLimit := seqtrack.GetSeqAck(serverConn)
	...
	connInfo := rawsock.ConnInfo{
		...
		Seq: seq, Ack: ack, HopLimit: hopLimit,
	}

	ttl := fakeTTL(h.mgr.cfg.FakeTTL, hopLimit)
	for i := 0; i < 3; i++ {
		if err := h.mgr.rawSock.SendFake(connInfo, fake.TLSClientHello, ttl); err != nil {
			h.mgr.logger.WithError(err).Warn("SendFake failed")
			break
		}
	}
	h.mgr.logger.WithFields(logrus.Fields{
		"dst": dst, "seq": seq, "ack": ack, "ttl": ttl, "hop_limit": hopLimit,
